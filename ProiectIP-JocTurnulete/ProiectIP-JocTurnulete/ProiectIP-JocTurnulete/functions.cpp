...
void moveTower(position source, position destination) {
	while (!empty(gameTable[source.x][source.y])) {
		pushToQueue(gameTable[destination.x][destination.y].level,
			popFromQueue(gameTable[source.x][source.y].level, gameTable[source.x][source.y].numberOfLevels),
			gameTable[destination.x][destination.y].numberOfLevels);
	}
	pushToQueue(gameTable[source.x][source.y].level, generateSquare(), gameTable[source.x][source.y].numberOfLevels);
}
